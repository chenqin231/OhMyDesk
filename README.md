...
| **M2 远程控制** | 模式 A（Web→终端）/ 模式 B（终端→终端）授权 → 画面 → 键鼠 → **远程命令 / 文件传输 / 即时消息**（会话内多标签，懒推流）→ 断开 | Agent + Web |
| **M3 批量监控** | 一键批量截图，在线终端屏幕墙 | Web + Agent |
| **M4 会话审计** | 连接/操作纯文本审计落 SQLite，按终端/时间筛选 | Server |
| **M5 MCP/AI** | 管控数据以 MCP 只读工具暴露给 AI，自然语言问答 | MCP + Web |

> **账号认证与按用户数据隔离（v0.5+）**：被控端客户端启动后需用 WEB 管理后台账号**登录**方可上线；
> 登录账号即为该终端**负责人（owner）**，服务端从校验通过的 JWT 派生归属（不信任客户端自报，防伪造）。
> 普通账号只能查看/远控/审计**自己负责的终端**，`superadmin` 全量可见可控。
> 归属由客户端登录自报（换绑：后登录者接管）；未登录旧端 `owner=NULL`，仅 `superadmin` 可见，现网不中断。
