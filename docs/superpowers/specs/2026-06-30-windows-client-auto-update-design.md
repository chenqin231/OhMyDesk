...
- **手测（真实环境）**：拉 `latest.json`+`.minisig`、gzip 下载解压、50MB 上限与超时、Windows self_replace + relaunch（含残留旧映像下次启动清理、令牌继承不二次弹 UAC、双实例自愈）、双会话+发起中门控（主控中/被控中/发起中不替换、皆空闲替换）、UpdateNotice UI 展示 + arboard 复制链接（非 Windows + Win 失败兜底）、发布脚本远端验收 + 原子切换。
