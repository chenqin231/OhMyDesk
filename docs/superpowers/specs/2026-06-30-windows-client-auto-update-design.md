...
启动延迟 ~30s + 每 6h（间隔可经环境变量覆盖）
 ...
 └─② GET <base>/latest.json   （≤64KB，连接超时 10s）
       ...
       └─⑤ 门控（should_update）：!enabled → 跳过；
             allow_downgrade=true → version != current 即继续（紧急回退/钉版，无灰度，见 §9）；
             否则 is_newer 且 (current < min_version 强制) 或 (bucket(endpoint_id) < rollout_percent) → 继续，否则跳过
       ...
       └─⑦ 末字节数 == manifest.size 且 SHA-256 == manifest.sha256 ？
             ├─ 否 → 删临时文件 + 告警 + 下周期重试
             └─ 是 → ⑧ 应用窗口：
                       检查 cur_session、ctrl_session、PENDING_CONNECT（主控发起中）三者皆空闲 → 置 UPDATING=true →
                       二次复检仍空闲（防竞态）→ self_replace(临时文件) → spawn 新 exe → 本体退出
                       （任一非空闲 → 解除 UPDATING，推迟到下周期）
```
...
```rust
/// 启动更新守护线程。endpoint_id 用于灰度确定性分桶；双会话 + 发起中态决定空闲。
pub fn spawn_update_daemon(server_url: String, endpoint_id: String, cur_session: SharedSession, ctrl_session: SharedSession);

/// 准入/发起侧查询：替换窗口内拒绝新会话与新发起。
pub fn is_updating() -> bool;          // 读 static AtomicBool UPDATING

/// 主控发起中标志：UI 发起回调置位、收 RemoteAck/Reject/超时清零；纳入空闲判定。
pub fn set_pending_connect(on: bool);  // 写 static AtomicBool PENDING_CONNECT
```

`spawn_update_daemon` 在 `main.rs` 用 `info.id.clone()` 传入 endpoint_id。**双向会话门控**（本模块对外仅此两处新增耦合）：

- **被控准入侧**：收到 ConnectRequest / 即将建立被控会话时查 `is_updating()`，UPDATING 则拒绝（回执「正在更新，请稍后」）。
- **主控发起侧**：UI 发起远控回调（`FromUi::StartRemote`）发送前查 `is_updating()`，UPDATING 则拒绝发起；否则 `set_pending_connect(true)`，并在收 RemoteAck/Reject 或发起超时时 `set_pending_connect(false)`。
- 守护线程**空闲判定** = `cur_session` 空 && `ctrl_session` 空 && `!PENDING_CONNECT`；专治「发起中（cur_session 尚未写入）窗口被替换重启」的竞态。
...
| `should_update(manifest, current, endpoint_id) -> bool` | enabled + allow_downgrade + 强制 + 分桶门控 | sha2 | ✅ |
...
self-replace = "1.5"      # crates.io 包名带连字符；代码内仍 `use self_replace`；Windows 运行中 exe 自替换
tempfile = "3"            # 安全临时文件落盘
url = "2"                 # 基址/同源解析
...
v1 不做进程级自动回滚，改用**发布侧三道闸**，回退闭环、无需现场重装：

- **灰度**：新版先 `rollout_percent: 10` 发布 → 观察少量客服机 → 逐步 50→100。坏包最多波及当前百分比。
- **暂停**：发现异常立即把 `enabled` 改 false 重签发布 → 全网秒级停更（阻止未中招机器继续升级）。
- **回退（自动）**：用**旧版本号 + 旧 sha256 + `allow_downgrade: true`** 重签 `latest.json`；已升到坏版的机器下个周期检测到 `version != current` 即自动降级回旧版（保留的上一版 exe 仍在 `/downloads/`）。回退完成后再发一版把 `allow_downgrade` 复位 false。

> `allow_downgrade` 是 v1 对「无进程级自动回滚」的等价替代：常态只升不降（`is_newer`），紧急时签名授权全网降级，回退不必触达每台客服机。保留上一版 exe 是回退前提（发布脚本默认不删旧 exe）。

---

## 10. 测试策略

- **单测（CI 可跑，纯函数/纯逻辑）**：`resolve_base`（wss 派生 / ws 拒绝 / 显式 https 覆盖 / 非 https 拒绝）、`Manifest::parse`（正常/缺 min_version/多余字段/超 64KB 拒绝）、`verify_manifest_sig`（合法签 / 错签 / 篡改 json）、`same_origin`（同源/异源/异端口）、`is_newer`、`should_update`（enabled=false / allow_downgrade 降级 / 强制 min_version / 分桶边界 0·99·阈值）、`bucket`（同 id 恒定、分布合理）、`verify_sha256` + size 双校验。
- **手测（真实环境）**：真实拉 `latest.json`+`.minisig`、gzip 下载解压、50MB 上限与超时、Windows self_replace + relaunch、双会话+发起中门控（主控中/被控中/发起中不替换、皆空闲替换）、UPDATING 拒新会话与新发起。
...
- **进程级**自动回滚（新版崩溃自检、进程自主回退）；v1 回退需运维签发 `allow_downgrade` 清单（非进程自主，但已闭环无需现场重装）。
