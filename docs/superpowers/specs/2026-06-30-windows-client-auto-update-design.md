...
启动延迟 ~30s + 每 6h（间隔可经环境变量覆盖）
 ...
 └─② GET <base>/latest.json（≤64KB，连接超时 10s）+ GET <base>/latest.json.minisig
       ...
       └─⑤ should_update：!enabled→跳过；allow_downgrade→version!=current 即继续（无灰度）；
             否则 is_newer 且 (current<min_version 强制) 或 (bucket(endpoint_id)<rollout_percent) → 继续
       └─⑥ asset.auto == false（Linux/macOS）→ 发 ToUi::UpdateNotice（版本/notes/url），结束，不下载不替换
             asset.auto == true（Windows）→ 继续：
...
新增 `ToUi::UpdateNotice { version: String, notes: Option<String>, url: String }`（客户端内部枚举，**非协议消息**，不碰 server/protocol）。`ui_glue::consume_to_ui` 收到后置 Slint 属性，UI 显示非侵入横幅「发现新版 vX.Y.Z — 下载」，点击经既有方式打开浏览器/复制链接。触发场景：① 非 Windows 发现新版；② Windows 自动更新连续失败兜底。
...
- `UpdateNotice` 复用既有 `ToUi` + Slint，**无新依赖**；`assets` map 为 serde 结构，**无新依赖**。
...
**B. 本机发布脚本 `packaging/download/publish-windows-update.sh`（私钥在本机运行）**：
...
4. 生成 `assets` map → `latest.json`（version、assets、enabled/rollout_percent/min_version/allow_downgrade、notes）。
...
- **手测（真实环境）**：拉 `latest.json`+`.minisig`、gzip 下载解压、50MB 上限与超时、Windows self_replace + relaunch、双会话+发起中门控（主控中/被控中/发起中不替换、皆空闲替换）、UpdateNotice UI 展示（非 Windows + Win 失败兜底）、发布脚本远端验收 + 原子切换。
...
带本更新器的**首版（拟 0.3.0）仍需手动装一次**（现网 0.2.1 无自更新逻辑，无法自我拉起）。0.3.0 装好后 Windows 全自动、其它平台得 UI 提示。一次性成本，无法绕过。发布 0.3.0 时同步：生成密钥对、公钥编入客户端、首跑 `publish-windows-update.sh` 产出带签清单 + 版本化产物。
