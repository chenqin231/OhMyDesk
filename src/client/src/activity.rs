...
impl ClientActivityState {
    pub fn new(cur_session: SharedSession, ctrl_session: SharedSession) -> Self {
        Self {
            cur_session,
            ctrl_session,
            pending_until_ms: AtomicU64::new(0),
    ...
    pub fn begin_pending_connect(&self, now_ms: u64) {
        self.pending_until_ms
            .store(now_ms + 30_000, Ordering::Release);
    }
    pub fn end_pending_connect(&self) {
        self.pending_until_ms.store(0, Ordering::Release);
    }
...
mod tests {
    ...
    fn st() -> (ClientActivityState, SharedSession, SharedSession) {
        ...
        (
            ClientActivityState::new(cur.clone(), ctrl.clone()),
            cur,
            ctrl,
        )
    }
