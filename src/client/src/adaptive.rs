...
pub fn store_level(l: u8) {
    LEVEL.store(l.min(MAX_LEVEL), Ordering::Relaxed);
}
...
pub fn is_overload(s: &WindowStats) -> bool {
    s.enc_p95_ms > ENC_OVERLOAD_P95_MS
        || (s.effective_fps < STARVE_FPS && s.dirty_p95 > STARVE_DIRTY_P95)
}
...
mod tests {
    ...
    fn resolve_enabled优先级() {
        assert!(
            !resolve_enabled(Some("0"), Some(true)),
            "env=0 关(压过 config)"
        );
